...
class Utils(CogBase):
    ...
    def __init__(self, bot: Latte):
        """
        Initialize the Cog :class Utils:
        :param bot: discord.py bot/client instance.
        """
    ...
    @commands.command(name="activity-info",
    ...
    async def create_vote(self, ctx: commands.Context, duration: float, *, vote_content: str):
        ...
        vote_embed = discord.Embed(title=f"[투표] {title}", description=desc, color=self.bot.initial_color)
